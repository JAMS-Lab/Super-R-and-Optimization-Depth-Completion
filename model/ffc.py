...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        # ffted = torch.fft.rfft(x, signal_ndim=len(fft_dim), normalized=self.fft_norm, onesided=False)
...
class FFCResnetBlock(nn.Module):
    def __init__(self,  in_channels, out_channels, ratio_gin, ratio_gout, enable_lfu=True, inline=False):
        super().__init__()
        self.conv1 = FFC_BN_ACT(in_channels, out_channels, kernel_size=3,
                                ratio_gin=ratio_gin, ratio_gout=ratio_gout,
                                activation_layer=nn.ReLU, enable_lfu=enable_lfu)

        self.conv2 = FFC_BN_ACT(out_channels, out_channels, kernel_size=3,
                                ratio_gin=ratio_gin, ratio_gout=ratio_gout,
                                activation_layer=nn.ReLU, enable_lfu=enable_lfu)
        self.inline = inline
